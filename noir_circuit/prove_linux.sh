...
CIRCUIT_NAME="shielded_pool_verifier"
TARGET_DIR="target"
...
if [ "$1" = "--paste" ] || [ "$1" = "-p" ]; then
    ...
    if [ ! -f "$PROVER_TOML" ]; then
        ...
        echo ""
        echo "Usage:"
        echo "  ./prove_linux.sh -p         # paste mode (paste + Ctrl+D)"
        echo "  ./prove_linux.sh            # use existing Prover.toml"
        echo "  ./prove_linux.sh file.toml  # load from file"
        exit 1
    fi
...
echo ""
echo "=== Shielded-Pool Proof Pipeline ==="
echo ""
...
# ── output hex ─────────────────────────────────────────────────────────
echo "-- shielded-pool proof hex --"
printf '0x'; xxd -p "$PROOF" | tr -d '\n'
echo ""
echo ""
echo "-- shielded-pool public witness hex --"
printf '0x'; xxd -p "$PW" | tr -d '\n'
echo ""

# ── audit proof output ───────────────────────────────────────────────
AUDIT_DIR="$SCRIPT_DIR/../audit_circuit/target"
AUDIT_PROOF="$AUDIT_DIR/rlwe_audit.proof"
AUDIT_PW="$AUDIT_DIR/rlwe_audit.pw"

if [ -f "$AUDIT_PROOF" ] && [ -f "$AUDIT_PW" ]; then
    echo ""
    echo "-- audit proof hex --"
    printf '0x'; xxd -p "$AUDIT_PROOF" | tr -d '\n'
    echo ""
    echo ""
    echo "-- audit public witness hex --"
    printf '0x'; xxd -p "$AUDIT_PW" | tr -d '\n'
    echo ""
else
    echo ""
    echo "(audit proof not found -- run generate_audit.py first)"
fi
